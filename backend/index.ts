...
import CookieExportImportService from "./services/cookie-export-import-service";
...
const cookieService = new CookieExportImportService();
...
// New Cookie Export Handler - Export cookies to local file
ipcMain.handle("profiles:exportCookieToFile", async (_event, { profileId, profileName }: { profileId: string; profileName?: string }) => {
  try {
    const result = await cookieService.exportCookies({
      profileId,
      profileName,
    ...
    console.error("Error in profiles:exportCookieToFile:", error);
    return {
      success: false,
      error: error.message || 'An unknown error occurred during cookie export.',
    };
  }
});

// Legacy Cookie Export Handler - Export cookies as JSON string (for backward compatibility)
...
// New Cookie Import Handler - Import cookies from local file
ipcMain.handle("profiles:importCookieFromFile", async (_event, { profileId, replace = false }: { profileId: string; replace?: boolean }) => {
  try {
    ...
    console.error("Error in profiles:importCookieFromFile:", error);
    return {
      success: false,
      error: error.message || 'An unknown error occurred during cookie import.',
    };
  }
});

// Get Cookie Statistics Handler
ipcMain.handle("profiles:getCookieStats", async (_event, profileId: string) => {
  try {
    const result = await cookieService.getCookieStats(profileId);
    return result;
  } catch (error: any) {
    console.error("Error in profiles:getCookieStats:", error);
    return {
      success: false,
      error: error.message || 'An unknown error occurred while getting cookie stats.',
    };
  }
});
